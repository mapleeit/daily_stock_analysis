...
def run_full_analysis(
    ...
    try:
        ...
        # === Optional: generate Feishu cloud document ===
        # Skip SDK import when not configured to avoid startup overhead and warning noise.
        feishu_doc_configured = bool(
            config.feishu_app_id and config.feishu_app_secret and config.feishu_folder_token
        )
        if feishu_doc_configured and (results or market_report):
            try:
                ...
                if feishu_doc.is_configured():
                    ...
                    # 1) Build title, e.g. "2026-02-25 18:00 大盘复盘"
                    tz_cn = timezone(timedelta(hours=8))
                    now = datetime.now(tz_cn)
                    doc_title = f"{now.strftime('%Y-%m-%d %H:%M')} 大盘复盘"

                    # 2) Build combined content (market review + stock dashboard)
                    full_content = ""

                    # Append market review section when available.
                    if market_report:
                        full_content += f"# 📈 大盘复盘\n\n{market_report}\n\n---\n\n"

                    # Append stock dashboard section generated by NotificationService.
                    if results:
                        dashboard_content = pipeline.notifier.generate_dashboard_report(results)
                        full_content += f"# 🚀 个股决策仪表盘\n\n{dashboard_content}"

                    # 3) Create cloud document.
                    doc_url = feishu_doc.create_daily_doc(doc_title, full_content)
                    if doc_url:
                        logger.info(f"飞书云文档创建成功: {doc_url}")
                        # Optional: notify channels with generated doc link.
